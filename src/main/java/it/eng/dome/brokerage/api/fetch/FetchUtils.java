...
public class FetchUtils {
	...
	/**
	 * Functional interface representing a fetcher that retrieves a list of objects of type {@code T}.
	 * <p>
	 * This interface is intended to be implemented with a lambda or method reference that
	 * fetches a subset of data based on specified fields, pagination parameters, and optional filters.
	 * </p>
	 *
	...
	public interface ListedFetcher<T> {
		/**
	     * Fetches a list of objects of type {@code T} according to the specified parameters.
	     *
	     ...
	     */
	    List<T> fetch(String fields, int offset, int limit, Map<String, String> filter);
	}
	

	/**
	 * Functional interface representing a processor that handles a batch of objects of type {@code T}.
	 * <p>
	 * This interface is intended to be implemented with a lambda or method reference that
	 * performs operations on a list of items, such as processing, transforming, or saving them.
	 * </p>
	 *
	 ...
	 * <p>
	 * This method retrieves items in pages of size {@code pageSize} using the provided fetcher,
	 * and returns a {@link Stream} that lazily iterates over all items, fetching new batches
	 * only when needed.
	 * </p>
	 *
	...
	/**
	 * Fetches elements in batches from a {@link ListedFetcher} and processes each batch using a {@link BatchProcessor}.
	 * <p>
	 * This method repeatedly fetches items in pages of size {@code batchSize} and passes each non-empty batch
	 * to the provided {@code consumer} for processing. Fetching continues until no more items are returned.
	 * </p>
	 *
	...
	/**
	 * Fetches all elements from a {@link ListedFetcher} and returns them in a single {@link List}.
	 * <p>
	 * This method retrieves items in batches of size {@code pageSize} using the provided {@code fetcher}.
	 * Each non-empty batch is added to a list which is returned at the end. The fetching continues
	 * until the fetcher returns no more items.
	 * </p>
	 * 
